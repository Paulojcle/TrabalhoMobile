    ...
    if (dados != null && mounted) {
      setState(() {
        ...
        if (dados['telefone'] != null &&
            dados['telefone'].toString().isNotEmpty) {
        ...
        if (isEntrada) {
          ...
          if (dataSelecionada.isAfter(_dataEntrada))
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Saída deve ser após a entrada.')),
            );
        }
      });
    }
    ...
    try {
      ...
      if (_cpfSalvo == null || _telefoneSalvo == null) {
        await _authService.atualizarDadosUsuario(
          uid: user.uid,
          dados: {'cpf': cpfFinal, 'telefone': telefoneFinal},
        );
      }

      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Reserva criada! ID: $reservaId')));
                    ...
                    onTap: () {
                      if (_quantidadeHospedes < widget.quarto.capacidade)
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text(
                              'Limite máximo de hóspedes atingido.',
                            ),
                          ),
                        );
                    },

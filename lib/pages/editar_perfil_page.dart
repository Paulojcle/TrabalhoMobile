...
class _EditarPerfilPageState extends State<EditarPerfilPage> {
  ...
  void initState() {
    ...
    _sobrenomeController = TextEditingController(
      text: widget.perfilAtual.sobrenome,
    );

    // NOVO: Inicializa vazio se for 'Não informado' para evitar a string no campo
    _cpfController = TextEditingController(
      text: widget.perfilAtual.cpf != 'Não informado'
          ? widget.perfilAtual.cpf
          : '',
    );
    _telefoneController = TextEditingController(
      text: widget.perfilAtual.telefone != 'Não informado'
          ? widget.perfilAtual.telefone
          : '',
    );
    _nascimentoController = TextEditingController(
      text: widget.perfilAtual.dataNascimento != 'Não informada'
          ? widget.perfilAtual.dataNascimento
          : '',
    );

    // 2. Regra de Imutabilidade
    if (widget.perfilAtual.cpf != 'Não informado' &&
        widget.perfilAtual.cpf.isNotEmpty) {
  ...
  Future<void> _selecionarData(BuildContext context) async {
    ...
    final DateTime dataMinimaPermitida = DateTime(
      hoje.year - 120,
      hoje.month,
      hoje.day,
    );
    final DateTime dataMaximaPermitida = DateTime(
      hoje.year - 18,
      hoje.month,
      hoje.day,
    );
    ...
    if (_nascimentoController.text.isNotEmpty) {
      try {
        ...
        if (parts.length == 3) {
          ...
          // Verifica se a data parseada está dentro dos limites de 120-18 anos
          if (parsedDate.isAfter(dataMinimaPermitida) &&
              parsedDate.isBefore(dataMaximaPermitida)) {
    ...
    // Define a data inicial final para o calendário
    ...
    if (picked != null) {
      setState(() {
        //  Salva a data selecionada no formato dd/mm/aaaa
  ...
  void _salvarAlteracoes() async {
    if (_isCpfEditable && _cpfController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("O CPF é obrigatório para o cadastro.")),
      );
      return;
    }
        ...
        TextFormField(
          ...
          decoration: InputDecoration(
            ...
            focusedBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(12),
              borderSide: const BorderSide(color: Color(0xFF192C50)),
            ),
            filled: true,
            fillColor: fillColor,
